 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_serial2 extends format_base {
    ...
    public function get_default_section_name($section) {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_serial2');
        } else {
    ...
    public function extend_course_navigation($navigation, navigation_node $node) {    
        ...
        $perm = new format_serial2\permission\course($USER->id, $COURSE->id);
        ...
        if($this->SURVEY_ENABLED === true && !$perm->isAnyKindOfModerator()){             
            ...
            foreach($records as $record){                
                if($DB->record_exists_sql('SELECT * FROM '.$CFG->prefix.'limesurvey_submissions WHERE user_id = ? AND survey_id = ?', array($USER->id, $record->survey_id)) === false){
                    ...
                    $redirectToSurvey = new moodle_url('/course/format/serial2/survey.php', array('c' => $COURSE->id));
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            $courseformatoptionsedit = array(
                ...
                'dashboardsectionexclude' => array(
                    'label' => get_string('dashboardsectionexclude', 'format_serial2'),
                    'element_type' => 'text'
                ),
            );
     ...
     *
     * In case if course format was changed to 'serial2', we try to copy options
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_serial2');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_serial2', $title);
        }
 ...
 * @return \core\output\inplace_editable
 */
function format_serial2_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            array($itemid, 'serial2'), MUST_EXIST);
