...
$PAGE->set_url($CFG->wwwroot.'/course/format/serial2/survey.php');
...
if(isset($sess->s)){
    ...
    unset($sess->s);  
    $url = new moodle_url('/course/format/serial2/survey.php');
    redirect($url);
} 
...
if(isset($sess->c)){  
    ...
    foreach($records as $record){    
        ...
        if($record->done === true){
            $state = "<span class=\"text-success font-weight-bold\">".get_string('surveyDone', 'format_serial2')."</span>";
        } else {
            $pending = true;
            if(isset($record->warndate) && is_int(+$record->warndate) && $record->warndate <= time()){
                $state = "<span class=\"text-danger font-weight-bold\">".get_string('surveyRequired', 'format_serial2')."</span>"; 
            } else {
                $state = "<span class=\"text-warning font-weight-bold\">".get_string('surveyPending', 'format_serial2')."</span>"; 
            }
        }     
    ...
    $PAGE->set_title(get_string('surveyTitle', 'format_serial2'));
    $PAGE->set_heading(get_string('surveyHeadline', 'format_serial2'));
    echo $OUTPUT->header();  
    echo '<div style="display: inline-block;"><p class="mt-2 mb-4">'.get_string('surveyDescription', 'format_serial2').'</p>';   
    echo "<table class=\"table\">
                    <thead class=\"thead-light\">
                        <tr>
                            <th scope=\"col\">".get_string('surveyID', 'format_serial2')."</th>
                            <th scope=\"col\">".get_string('surveyTitle', 'format_serial2')."</th>
                            <th scope=\"col\">".get_string('surveyStop', 'format_serial2')."</th>
                            <th scope=\"col\">".get_string('surveyState', 'format_serial2')."</th>
                            <th scope=\"col\">".get_string('surveyLink', 'format_serial2')."</th>
                        </tr>
                    <thead>
                    <tbody>
                        {$list}
                ...
                // <th scope=\"col\">".get_string('surveyStart', 'format_serial2')."</th><th scope=\"col\">".get_string('surveyWarn', 'format_serial2')."</th>
                                
    $courseURL = new moodle_url('/course/view.php', array('id' => +$sess->c));
    echo '<p class="text-center">'.get_string('surveyReqText', 'format_serial2').'</p>';
    echo '<div class="text-center"><button onClick="javascript:window.location.href=\''.$courseURL->__toString().'\';" class="btn btn-secondary center-block">'.get_string('surveyButton', 'format_serial2').'</button></div></div>';
    echo $OUTPUT->footer();
} else {    
