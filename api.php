...
class format_serial2_external extends external_api
{
    // Analytics
    ...
    public static function analytics($courseid)
    {
        ...
        try {
            global $USER, $CFG, $DB;
            $permission = new format_serial2\permission\course((int)$USER->id, $courseid);
            ...
            foreach ($enrollments as $user) {
                try {
                    ...
                    $sql = 'SELECT t.milestones, t.settings, t.timemodified 
                            FROM ' . $CFG->prefix . 'serial2_milestones AS t
                            WHERE   
                    ...
                    $surveyDone = $DB->get_record("user_preferences", array(
                        'name' => 'serial2_survey_done-course-' . (int)$courseid,
                        'userid' => (int)$user->id
                    ));
                    $surveyData = $DB->get_record("user_preferences", array(
                        'name' => 'serial2_survey_results-course-' . (int)$courseid,
                        'userid' => (int)$user->id
                    ));
    ...
    public static function limesurvey($courseid)
    {
        ...
        $out = array();
        try {
            $perm = new format_serial2\permission\course($USER->id, $courseid);
            ...
            foreach ($records as $record) {
                ...
                if ($DB->record_exists_sql('SELECT * FROM ' . $CFG->prefix . 'limesurvey_submissions WHERE user_id = ? AND survey_id = ?', array($USER->id, $record->survey_id)) === false) {
                    ...
                    //$url = new moodle_url('/course/format/serial2/survey.php', array('c' => $courseid));
    ...
    public static function updateuser($data)
    {
        ...
        try {
            ...
            if (!is_null($data->milestones)) {
                ...
                $transaction = $DB->start_delegated_transaction();
                $res = $DB->insert_record("serial2_milestones", $r);
                $transaction->allow_commit();
            ...
            if (!is_null($data->plan)) {
                ...
                func("serial2_survey_results", (int)$data->courseid, (int)$userid, $data->plan);
                func("serial2_survey_done", (int)$data->courseid, (int)$userid, 0);
            }
        } catch (Exception $ex) {
            $out['debug'] = $ex->getMessage();
        }
  ...
  public static function logger_parameters() {
    ...
    */
    ...
    public static function logger($data)
    {
        ...
        $r = new stdClass();
        $r->name = 'format_serial2';
        $r->component = 'format_serial2';
        $r->eventname = '\format_serial2\event\\' . $data['action'];
        $r->action = $data['action'];
        $r->target = 'course_format';
        $r->objecttable = 'serial2';
        $r->objectid = 0;
        $r->crud = 'r';
        $r->edulevel = 2;
        $r->contextid = 120;
    ...
    */
    ...
    public static function setmilestones($data)
    {
        ...
        $transaction = $DB->start_delegated_transaction();
        $res = $DB->insert_records("serial2_milestones", array($r));
        $sql = '
            INSERT INTO ' . $CFG->prefix . 'serial2_milestones (user,course,milestones,settings,timemodified) 
    ...
    public static function getmilestoneplan($param)
    {
        ...
        $sql = '
            SELECT milestones 
            FROM ' . $CFG->prefix . 'serial2_milestone_plans AS t
            WHERE   
                t.course = ? 
                AND t.plan = ?
    ...
    public static function setmilestoneplan($param)
    {
        try {
            ...
            if ($meta->user->loggedin === true && ($meta->user->manager === true || $meta->user->coursecreator === true || $meta->user->siteadmin === true)) {
                ...
                $sql = 'SELECT id FROM ' . $CFG->prefix . 'serial2_milestone_plans WHERE course = ? AND plan = ? LIMIT 1';
                $transaction = $DB->start_delegated_transaction();
                $params = array();
                ...
                if ($count !== 0) {
                    ...
                    $c->id = $id->id;
                    $transaction = $DB->start_delegated_transaction();
                    $res = $DB->update_record("serial2_milestone_plans", $c);
                    $transaction->allow_commit();
                    if ($res === true) {
                        $data['success'] = true;
                    } else {
                    ...
                    $transaction = $DB->start_delegated_transaction();
                    $res = $DB->insert_records("serial2_milestone_plans", array($c));
                    $transaction->allow_commit();
                    $data['success'] = true;
                }
            } else {
    ...
    public static function userpreferences($data)
    {
        ...
        if ($exists != true) {
            ...
            //$sql = 'UPDATE '. $CFG->prefix .'user_preferences SET value=\''. $data['value'] .'\' WHERE name=\'serial2_survey_done\' ;';
...
;
    ...
    public static function reflectionRead($data)
    {
        ...
        $res = $DB->get_records_sql(
            "SELECT * FROM {serial2_reflections} WHERE courseid=:course AND userid=:user ORDER BY timecreated ASC",
    ...
    public static function reflectionCreate($data)
    {
        ...
        $transaction = $DB->start_delegated_transaction();
        $res = $DB->insert_record("serial2_reflections", $r);
        $transaction->allow_commit();
