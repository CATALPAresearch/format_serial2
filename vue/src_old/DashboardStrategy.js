                    ...
                    <div id="strategy-description" class="col-6">
                        ...
                        <div v-if="mode=='strategy'">
                            ...
                            <div v-if="currentStrategy=='mindmap'">
                                Mit Mindmaps können Sie zentrale Themen, Bezüge und Zusammengänge grafisch darstellen und visuell veranschaulichen. Bei einer
                                Mindmap wird die zentrale Idee eines Textes bzw. der zentrale Begriff, in der Mitte des Blattes platziert. Weitere Schlüsselwörter, die
                                im Text behandelt werden, werden nun in Relation dazu (Abstand - Nähe; Schriftgröße, etc.) hinzugefügt. Sie können diese Begriffe auch
                                durch Symbole oder Kurzkommentare ergänzen. Es empfiehlt sich allerdings, nicht ganze Sätze zu formulieren, da die Übersichtlichkeit
                                darunter leiden könnte. </div>
                            <div v-if="currentStrategy=='exzerpt'">
                                Ein Exzerpt ist mehr als nur eine einfache Zusammenfassung der wichtigsten Inhalte. Gerade wenn es darum geht, vor einer Prüfung
                                Lerninhalte noch einmal zu wiederholen, helfen Ihnen Exzerpte oder Zusammenfassungen dabei, schnell in die einzelnen Wissensbereiche
                                einzutauchen, ohne auf das ausführliche Kursmaterial zurückzugreifen. Zudem können Sie schon bei der Erstellung eines Exzerpts üben,
                                Wissen in eigenen Worten wiederzugeben. Auch kritische Perspektiven und eine wissenschaftliche Schreibweise sollten Sie in einem Exzerpt
                                berücksichtigen.
                            </div>
                            <div v-if="currentStrategy=='toc'">
                                Gliederungen helfen, einen Überblick über den zu lernenden Inhalt zu bekommen. Wissen kann so leichter strukturiert oder kategorisiert
                            ...
                            <div v-if="currentStrategy=='fastreading'">
                                ...
                                die nachfolgenden Abbildungen dargestellt:<img class="w-50" :src="moodlePath + \'/course/format/serial2/pix/schnelle_Lesebewegung.png\'" /><img
                                    class="w-50" :src="moodlePath + \'/course/format/serial2/pix/normale_Lesebewegung.png\'" />Eine weitere unterstützende Technik bietet die
                            ...
                            <div v-if="currentStrategy=='readingcomprehension'">
                                ...
                                schleichen sich damit zusätzliche gedankliche Blockaden ein. Folgende Tipps helfen Ihnen jedoch vielleicht weiter:<ul>
                                    <li>Schlagen Sie Fachbegriffe, die Ihnen noch fremd sind, nach und überlesen Sie diese nicht.</li>
                                    <li>Ändern Sie Ihre Lesetechnik.</li>
                                    <li>Dokumentieren Sie Ihren Lernfortschritt und zwar Abschnitt für Abschnitt. Im Laufe der Zeit werden Sie vermutlich merken, dass
                                        Sie mehr verstanden haben, als Sie zunächst dachten.</li>
                                    <li>Greifen Sie zu Sekundärliteratur. Manchmal muss ein Sachverhalt nur mit anderen Worten erklärt werden und schon versteht man ihn
                                        besser.</li>
                                    <li>Bilden Sie Lerngruppen und versuchen Sie anderen zu erklären, was Sie bereits verstanden haben.</li>
                                </ul>Auch wenn es am Anfang mühsam ist: Bleiben Sie am Ball und lassen Sie sich auch von auftretenden Problemen nicht abschrecken. Gehen
                                Sie in den Austausch mit anderen und nutzen Sie die Diskussionsforen in der Lernumgebung. Ihren Kommilitonen und Kommilitoninnen wird es
                                nicht anders gehen als Ihnen!
                            </div>
                            <div v-if="currentStrategy=='transfertoknown'">
                                Wenn Sie sich neue Wissensschemata aneignen möchten, können diese schneller bzw. einfacher gelernt werden, wenn Sie diese mit Beispielen
                                in der eigenen Praxis oder dem eigenen Arbeitsumfeld in Verbindung bringen. Zudem kann dieses auf eine Generalisierbarkeit hin überprüft
                            ...
                            <div v-if="currentStrategy=='subjectrelations'">
                                Manche Inhalte kennen Sie ggf. aus einem anderen Kurs oder sie kommen einem irgendwie bekannt vor. In diesem Fall können Sie auf das
                            ...
                            <div v-if="currentStrategy=='reminder'">
                                Mit einem Reim oder einer Eselsbrücke können Sie sich Begriffe oder Reihenfolgen einfacher merken.
                            </div>
                            <div v-if="currentStrategy=='remindercomplex'">
                                Die Loci-Methode oder auch der Lernspaziergang sind Methoden, um sich Dinge in einer konkreten Reihenfolge einzuprägen. Mit der Loci
                                Methode können Sie sich komplexe Dinge wie z. B. Prozesse oder Stufenmodelle schneller merken, indem Sie Lerninhalte mit
                                Gegenständen oder Orten und Abfolgen aus dem Alltag verknüpfen. Diese Methode ist auch als Gedächtnisspaziergang bekannt.
                            </div>
                            <div v-if="currentStrategy=='planning'">
                                Bei der Planung kommt es darauf an Anforderungen zu analysieren, Lernziele zu formulieren und passende Lernstrategien für die
                                Umsetzung auszuwählen. Der Meilensteinplaner in dieser Lernumgebung soll Sie dabei unterstützen. In einem Meilenstein können Sie ein
                                Lernziel und die Anforderungen dazu festhalten. Über das Menü können Sie dann Aktivitäten und Materialien aus der Lernumgebung
                                auswählen oder individuelle Arbeitsmaterialien vermerken.
                            </div>
                            <div v-if="currentStrategy=='prepare'">
                                Was gehört zu einer guten Vorbereitung? Zunächst einmal sollten Sie sich ein anregendes Arbeitsumfeld schaffen und dafür einen
                                geeigneten Ort wählen.Dieser Ort wird in den nächsten Wochen und Monaten Ihr ganz persönlicher Arbeitsplatz sein.Viele Studierende
                                können besser lernen, wenn sie dafür einen Ort auswählen, den sie mögen und der für sie positiv besetzt ist.Dies kann beispielsweise
                                die Bibliothek in der Nähe sein oder ein ruhiges Arbeitszimmer, in dem man schon andere Dinge bearbeitet hat, die zum Erfolg
                                führten.Ihr Arbeitsbereich sollte möglichst störungs- und ablenkungsfrei sein und Sie sollten die notwendige Ausstattung
                                eingerichtet oder griffbereit haben.Erzwungene Lernpausen, weil man erst den Stift suchen muss oder der Textmarker fehlt, sind
                                ärgerlich und unnötig.Überlegen Sie also, welche Materialien Sie für Ihren Arbeitsprozess brauchen und legen Sie sich diese vorab
                            ...
                            <div v-if="currentStrategy=='regulations'">
                                Regulationsstrategien dienen in der Regel der Identifikation von Verständnislücken und tragen zum Ergreifen von Maßnahmen zur
                                Schließung der Lücken bei. Es gibt verschiedene Ansätze, um Ihr Lernen zu regulieren. Dabei gestaltet sich dieser Prozess in der
                                Regel sehr individuell für eine*n Lernenden. Man kann nicht mit Bestimmtheit sagen, dass nur bestimmte Lernstrategien direkte
                                positive Wirkungen auf den Studienerfolg haben. Vielmehr gibt es auch die Überlegung, dass Sie je nach Lerntyp mit unterschiedlichen
                                Lernstrategien zum Studienerfolg kommen können. Insofern erscheint es erforderlich, den Strategieeinsatz zu planen, zu reflektieren
                                und ggf. anzupassen. Überprüfen Sie, welche Prüfungsanforderungen gegeben sind (z. B. reines Faktenwissen auswendig lernen oder
                                Zusammenhänge erkennen, Wissen auf neue Anwendungsgebiete zu transferieren), auf welche Ressourcen Sie zurückgreifen können (z. B.
                                gemeinsames Lernen mit Kommilitonen*innen) und planen Sie die Zeit für den Lernaufwand bzw. die konkrete Prüfungsvorbereitung ein.
                                Reflektieren Sie außerdem, welche Lernerfahrungen Sie bisher gemacht haben und welche Lernstile und -strategien Sie bevorzugen.
                                Überlegen Sie, wie und in welchen Situationen Sie am besten lernen. Welche Lernstrategien Sie nutzen, hängt nicht zuletzt auch davon
                                ab, welche Lernerfahrungen Sie machen. Wenn Sie merken, dass Sie besonders gut gemeinsam mit anderen Kommilitonen/-innen lernen
                                können, werden Sie das kooperative Lernen als Lernstrategie womöglich im weiteren Verlauf Ihres Studiums beibehalten. Diese
                                Lernumgebung unterstützt Sie, indem zum Abschluss jedes Meilensteins eine Reflexion angeboten wird. Die Erkenntnisse, die Sie sich
                                in diesem Reflexionsprozess im Freitextfeld notieren werden für Sie hier im Bereich Lernstrategien für Sie festgehalten. So können
                                Sie die Entwicklung Ihrer Lernstrategien selbst beobachten und nachvollziehen.
                            </div>
                            <div v-if="currentStrategy=='goals'">
                                Informationen zu den Prüfungsanforderungen finden Sie in der Regel im Modulhandbuch bzw. im Studienportal und in der
                                Moodle-Lernumgebung des jeweiligen Moduls. Große Lernziele stellen beispielsweise das Bestehen einer Prüfung oder ein zügiger
                                Fortschritt im Studium dar.Die Arbeit an diesen Zielen erstreckt sich zudem über einen recht langen Zeitraum, der von Ihnen vor
                                allem Disziplin und Durchhaltevermögen verlangt.Um Ihr großes Ziel zu erreichen, macht es daher Sinn, dieses in kleinere - schneller
                                zu erreichende - Ziele aufzuteilen. Kleinere Lernziele können zum einen die Lerninhalte sein, die Sie für ein Modul
                                erarbeiten.Hierzu bieten einige Module eine zeitliche Strukturierung oder einen Lesekurs an, mit denen Sie dann systematisch durch
                                das Studienmaterial geleitet werden.Zusätzlich können Sie sich auch ganz individuelle Ziele setzen.Kleinere Ziele können zum
                            ...
                            <div v-if="currentStrategy=='demand'">
                                ...
                                seinen/ihren Fokus aufgrund des Vorwissens gerichtet hat. Der Scheinwerfer, der zur neuen Erkenntnis führt, ist also immer auf das
                                Vorwissen zurückzuführen (Popper,1984, S.360).Im Umkehrschluss bekennt Lehner(2015): „Wer ein bestimmtes Wissen – dazu gehören
                                faktische Details genauso wie Definitionen oder Modelle – verfügbar hat, der kann weitergehende Informationen ableiten
                                bzw.rekonstruieren.“ (S.16) Faktenwissen ist also wichtig, um überhaupt bestimmte Sachverhalte erst wahrnehmen und erfassen zu
                                können.Dass Studierende in der Lage sind, ihr Lernverhalten jederzeit zwischen Auswendiglernen und Verstehen anzupassen, erkannten
                                auch die Psychologen Marton & amp; Säaljö(1976).Ein solcher Wechsel kann jederzeit stattfinden: wenn zum Beispiel ein Stoff als zu
                                schwierig empfunden wird oder das Zeitbudget schwindet, wird Auswendiglernen dem Verständnislernen vorgezogen.Verstehen wiederum ist
                                umso leichter, je mehr auf ad hoc verfügbares Wissen zurückgegriffen werden kann.So entstehen Verknüpfungen zwischen Bekanntem und
                                Neuem, welche neues Wissen konstruieren(vgl.Lehner,2015).Es verlangt also weniger Anstrengung etwas zu verstehen, wenn man auf
                                bereits Gelerntes aufbauen kann.Genauso kann es effektiver sein, Dinge, die man noch nicht versteht, erst einmal auswendig zu lernen
                                und dann später das Verständnis zu rekonstruieren.Ein wechselseitiger Prozess aus Auswendiglernen und Verstehen führt also zu
                                geringerer Anstrengung.Um etwas zu lernen, bedarf es jedoch noch weitere Schritte.Steiner(2013) formulierte treffend: „Seien wir
                                realistisch.Der Lernprozess ist noch nicht abgeschlossen, wenn wir ein Kapitel erarbeitet und verstanden haben.“ (S. 243).In einer
                                Prüfung muss man in der Regel das Gelernte auch wiedergeben oder erklären können.Verständnis ist also eine wesentliche Voraussetzung
                                des Lernens, doch erst durch das Wiedergegebene kann Gelerntes auch demonstriert und für ein tiefergehendes Verständnis genutzt
                                ...
                                <ul>
                                    ...
                                    <li>Popper, K. R. (1984). Objektive Erkenntnis (4. Aufl.). Hamburg: Hoffmann &amp; Campe.Steiner, V. (2013). Konzentration
                                        leicht gemacht: Die wirksamsten Methoden für Studium, Beruf und Alltag. München: Piper.</li>
                                    </ul>
                            </div>
                            <div v-if="currentStrategy=='attention'">
                                <p>Grundsätzlich ist es im Studium von großem Vorteil für Sie, wenn Sie kontinuierlich lernen. Verstehen ist weniger anstrengend, wenn
                                Sie auf bereits Bekanntem aufbauen können. Verteiltes und wiederholendes Lernen ist dabei effektiver als das so genannte
                                Bulimie-Lernen. Am Tag gibt es zudem Zeiten, zu denen man sich besser konzentrieren kann und Zeiten, zu denen das fast unmöglich
                                erscheint. Anhand der folgenden Fragen können Sie für sich selbst überlegen, wann Ihnen Lernen wahrscheinlich leichter fällt und
                                welche Zeit Sie wann zum Lernen benötigen.
                                </p>
                                <ul>
                                    <li>Wie lange können Sie sich konzentriert und fokussiert mit einem Text aus einem Studienbrief beschäftigen?</li>
                                    <li>Zu welchen Zeiten haben Sie genügend Energie, um sich mit neuen Inhalten auseinander zu setzen?</li>
                                    <li>Wann ist es Ihnen voraussichtlich auf keinen Fall möglich, Ruhe zum Lernen zu finden?</li>
                                </ul>
                                <p>
                                Im Verlauf Ihres Studiums werden Sie vermutlich Ihre Aufmerksamkeitsspanne steigern können, wenn Sie sich regelmäßig darin
                                üben, sich zu fokussieren.
                                </p>
                                <p><strong>Multitasking – geht das wirklich?</strong>
                                </p><p>Bereits 1997 stellten Bujon und Quaireau fest, dass eine
                                durch unterschiedliche Einflüsse geteilte Aufmerksamkeit zu einer verminderten Leseleistung führt. Die Forscher unterschieden in
                                einem Versuch zwischen dem Einfluss von klassischer Musik, einer gesprochenen Tontonspur sowie eines Videos als Störquellen in Bezug
                                auf die Leseleistung von Probanden. Sie stellten fest, dass Musik kaum die Leseleistung beeinflusst hat (&lt;5%), während das
                                gleichzeitige Hören einer gesprochenen Sprache die Leseleistung bereits zu einem Viertel verringerte. Bei der Präsentation eines
                                Videos während des Leseprozesses lag die Minderungsquote sogar bei 40% (vgl. Boujon, Quaireau, 1997).Andere Forschungen legen dar,
                                dass Personen, die häufig Multitasking praktizieren, leichter durch äußere Reize abgelenkt werden, wohingegen Personen, die weniger
                            ...
                            <div v-if="currentStrategy=='resourcemanagement'">
                                ...
                                werden.Sie sollten nicht zu den alltäglichen Dingen gehören.Neben dem Lernen ist es zum Beispiel auch wichtig, Stress abzubauen und
                                so wieder aufnahmefähig für Neues zu werden.Gönnen Sie sich, Ihrem Geist und Körper also regelmäßig Sport und Entspannung.Besonders
                            ...
                            <div v-if="currentStrategy=='timemanagement'">
                                ...
                                <ul>
                                    <li>Individuelles Zeitkontingent: Vollzeit- oder Teilzeitstudium, Vereinbarkeit Studium und Beruf</li>
                                    ...
                                    <li>Individuelle Ziele: Weiterbildung, persönliche Entwicklung, Notenwunsch, Studiendauer</li>
                            ...
                            <div v-if="currentStrategy=='effectivetimemanagement'">
                                <p>Eine sehr effektive Methode des Zeitmanagements ist die Pomodoro-Technik, die in den 1980er Jahren von Francesco Cirillo
                                    ...
                                    Kurzzeitwecker aus seiner Küche, der die Form einer Tomate (ital. Pomodoro) hatte. Francesco Cirillo arbeitete sich so an einem
                                    Vormittag zunächst durch vier "Pomodori" (plural von Pomodoro), bevor er eine längere Pause machte.</p>
                                <p>Nach dieser Technik planen Sie je nach Priorität den aktuellen Tag und visualisieren das, was Sie geschafft haben. Ein wichtiges
                                    Ziel der Technik ist zudem die Reduktion von Unterbrechungen. Sowohl interne (Abschweifen) als auch externe Störungen sollten
                                ...
                                <ul>
                                    ...
                                    <li>Stellen Sie nun den Wecker und setzten Sie sich selbst Ihr Ziel, das Sie in 25 Minuten ohne Unterbrechung erreichen möchten.
                                    </li>
                                    <li>Los geht's! Bearbeiten Sie die Aufgabe bis der Wecker klingelt.</li>
                                    <li>Geschafft, diese „Pomodoro" kann abgehakt werden!</li>
                                    <li>Nun machen Sie eine kurze Pause (5 Minuten) und dann geht es weiter mit der nächsten „Pomodoro".</li>
                            ...
                            <div v-if="currentStrategy=='literature'">
                                Weiterführende Literatur ist im Studium eine wichtige Ressource. Schlagen Sie unbekannte Fachbegriffe nach, oder schließen Sie
                                Verständnislücken, indem Sie zu weiterführender Literatur greifen, die Ihnen den Sachverhalt z. B. aus einer anderen Perspektive
                            ...
                            <div v-if="currentStrategy=='timethefts'">
                                Die süßen kleinen „Zeitdiebe" können als objektive oder subjektive Störungen verstanden werden.<ul>
                                    ...
                                    <li>Sie kommen zu uns - Lärm; Kinder; Besuch; der Partner; das Haustier oder die Sonderangebote, die ihr Browser für Sie
                                        gefunden hat</li>
                                    <li>Sie verkleiden sich als der innere Schweinehund -Unlust; wenig Disziplin; Gedanken an Unerledigtes; nicht NEIN sagen können;
                                        Beziehungsstress oder Hilfsbereitschaft</li>
                                    <li>Oder plötzlich und unerwartet haben sie sich angebahnt - Verspätungen im ÖPV; Termindruck; Zeitdruck, weil man zu viel
