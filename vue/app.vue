<template>
    <div>
        ...
        -->
        <!-- Questionnaire modal -->
        <div v-if='surveyRequired && aple1801.includes(courseid)' class='modal fade' id='questionnaireModal'
        ...
        <div class='row mb-3 form-group'>
            <div class='col-8'>
                ...
                <div style='background-color:#e9f5f9; color:#333; display:inline-block; width:140px; height:20px;'>
                    <select id='select-goal' @change='switchGoal($event)' class='pt-1 pb-1 fa-caret-down lad-select mr-0'
            ...
            <div v-if='$store.state.policyAccepted' class='col-md-12'>
                <ul class='nav nav-tabs dashboard-tab'>
                    <li class='nav-item active'>
                        <router-link to='/' class='nav-link' v-on:click='log('
                            dashboard_overview_open',0)'>Übersicht</router-link>
                        <a hidden class='nav-link active' v-on:click='log(' dashboard_overview_open',0)' data-toggle='tab'
                            href='#learningoverview' role='tab'>Übersicht</a>
                    </li>
                    <li class='nav-item'>
                        <router-link to='/completion' class='nav-link' v-on:click='log('
                            dashboard_completion_open',0)'>Genutzte Lernangebote</router-link>
                        <a hidden class='nav-link' v-on:click='log(' dashboard_completion_open',0)' data-toggle='tab'
                            href='#learningstatus' role='tab'>Genutzte Lernangebote</a>
                    </li>
                    <li class='nav-item'>
                        <router-link to='/strategies' class='nav-link' v-on:click='log('
                            dashboard_strategy_open',0)'>Lernstrategien</router-link>
                        <a hidden class='nav-link' v-on:click='log(' dashboard_stratgy_open',0)' data-toggle='tab'
                            href='#learningstrategy' role='tab'>Lernstrategien</a>
                    </li>

                    <li hidden class='nav-item '>
                        <a class='nav-link' v-on:click='log(' dashboard_time-management_open',0)' data-toggle='tab'
                            href='#timemanagement' role='tab'>Zeitmanagement</a>
                    </li>
                    <li hidden class='nav-item'>
                        <a class='nav-link' v-on:click='log(' dashboard_strategy_open',0)' data-toggle='tab'
                            href='#strategy' role='tab'>Lernen gestalten</a>
                    </li>
                    <li hidden class='nav-item'>
                        <a class='nav-link' v-on:click='log(' dashboard_communication_open',0)' data-toggle='tab'
                            href='#communication' role='tab'>Kommunikation</a>
                    </li>
                    <li hidden class='nav-item'>
                        <a class='nav-link' v-on:click='log(' dashboard_assessment_open',0)' data-toggle='tab' href='#quiz'
                            role='tab'>Quiz</a>
                    </li>
                </ul>
                <br>
                <router-view style='display:block;' v-on:log='log' v-bind:course='course' v-bind:aple1801='aple1801'
                    v-bind:currentGoal='currentGoal' v-bind:surveyRequired='surveyRequired'
                    v-bind:surveyLink='surveyLink'></router-view>
            </div>
        </div>
        <hr class='mb-3 mt-3' />
        ...
        <div v-if='aple1801.includes(courseid)' class='mb-3 mt-3' style='width:100%;height:auto;'>
            <video controls='true' style='width:100%;max-height:400px;'>
                ...
                <source v-if='controlgroup' src='https://nise81.com/1801-Intro-SS2023_Kontrollgruppe.mp4' type='video/mp4'>
                </source>
...
<style>
ul.dashboard-tab li.nav-item a,
ul.dashboard-tab.nav-tabs a {
    border: none;
